...
class Tracker:
    ...
    def _add_track(self, frame_id, bbox, detection_confidence, class_id, **kwargs):
        ...
        self.tracks[self.next_track_id] = Track(
            ...
            data_output_format=self.tracker_output_format, display=True,
            **kwargs
        )
    ...
    def _update_track(self, track_id, frame_id, bbox, detection_confidence, class_id, display, lost=0, iou_score=0., **kwargs):
        """
        Update track state.
        ...
        self.tracks[track_id].update(
            frame_id, bbox, detection_confidence, class_id=class_id, lost=lost, iou_score=iou_score, display=display, **kwargs
        )
        ...
        if len(track_ids):
            ...
            for idx in track_indices:
                ...
                if len(remaining_detections):
                    ...
                    #print(self.tracks[track_id].display)
                    self._update_track(track_id, self.frame_count, bbox, confidence, class_id=class_id, display=self.tracks[track_id].display)
                    #print("---->>",self.tracks[track_id].display)
